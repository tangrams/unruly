...
function mergeWithDepth(matchingTrees, context) {
  var style = {visible: true};
  var deepestOrder;
  ...
  for (var x = 0; x < len; x++) {
    for (var t = 0; t < matchingTrees.length; t++) {
      var treeStyle = matchingTrees[t][x];
      if (!treeStyle) {
        continue;
      }
      for (var key in treeStyle) {
        if (key === 'visible') {
  ...
  if (deepestOrder !== undefined) {
    var matchingOrderTree = matchingTrees[deepestOrder];
