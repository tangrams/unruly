...
Object.defineProperties(module.exports, {
  ...
  mergeWithDepth: {get: function() {
      return mergeWithDepth;
    }},
...
function mergeWithDepth(matchingTrees) {
  var properties = {};
  var style = {};
  var deepestOrder;
  var orderReset = 0;
  for (var x = 0; x < matchingTrees.length; x += 1) {
    var tree = matchingTrees[x];
    for (var i = 0; i < tree.length; i += 1) {
      var style$__8 = tree[i];
      for (var key in style$__8) {
  ...
  for (var prop in properties) {
    ...
    if (prop === 'visible') {
      style[prop] = !properties[prop].some((function(x) {
  ...
  if (deepestOrder) {
    var matchingOrderTree = matchingTrees[deepestOrder.position];
    var orders = matchingOrderTree.filter((function(x) {
      return x.order;
    })).map((function(x) {
      return x.order;
    }));
...
($traceurRuntime.createClass)(RuleTree, {
  ...
  findMatchingRules: function(context) {
    ...
    if (rules.length > 0) {
      if (flatten === true) {
        styles = [mergeWithDepth(rules.map((function(x) {
          return x.calculatedStyle;
        })))];
      } else {
        styles = rules.map((function(x) {
          return mergeStyles(x.calculatedStyle);
        }));
      }
    }
    return styles;
